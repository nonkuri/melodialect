...
- **20 のダイアレクト** — Chromatic（半音階クリシェと旋律的なベース）、Modal（モード中心の旋律と♭VII）、Pedal（持続音とsus/add9）をはじめ、Twilight / Angular / Orchestral / Bossa / Ostinato / Serene / Flow / Blue / Lament / Interlock / Voicing、教会旋法と変拍子の Drift（ドリアン）/ Pulse（4つ打ち138BPM）/ Ember（フリギア・7/8）/ Prism（リディアン・5/4）、日本の音階の Ryukyu（琉球音階・三線）/ Miyakobushi（都節音階・持続低音）を収録
...
- **独立したベース作曲** — ルート、ペダル、ウォーキング、オスティナート、対旋律の役割をセクションごとに選び、次のコード、メロディ、キックを参照してラインを生成
- **自動編曲と伴奏の多様化** — ピアノ、ギター、ドラムの参加、密度、音域、奏法、フィル、ブレイクを曲全体で計画。「伴奏の厚さ」と「展開」だけで簡単に調整でき、従来の手動パターン指定も維持
- **候補生成と比較** — 通常生成は複数案から破綻と類似を避けた1案を自動選択。「他の候補」では同じシードの差別化された案を、音楽的な差分表示と同じ再生位置でA/B比較
- **部分再生成と直接編集** — セクションの全体／メロディ／コード／ベース／伴奏を個別に変奏し、対象外パートを保持。セクション・小節・コード・ノートのロック、ピアノロール編集、コード編集、Undo／Redoも利用可能
...
- **内蔵の音色セット** — 琉球（歌三線と太鼓）、和（箏と尺八）、歌もの、エレクトリック・ポップ、ジャズ・コンボ、オーケストラ、ラテンの7種を同梱。高音質音源を準備してあれば、5パートの音色と音量補正をひとつ選ぶだけで着せ替えられる
...
高音質音源を保存したあとは、**ミキサーと音色 → 音色セットを選ぶ…**（または **音源を追加 / 管理**）から開く **内蔵の音色セット** で、5パートまとめて楽器を差し替えられます。音量はGeneralUser GSのプリセット間の音量差を実測して補正済みで、ミュート・ソロ・パンの設定は保持されます。いま当たっている音色セットの名前はミキサーに表示され、1パートでも差し替えると「カスタム」に変わります。
...
7. 必要なら **他の候補** で同じシードの別案を比較する
8. MIDI、WAV／WAVステム、MusicXML、テキストのいずれかで書き出す

詳しい操作は [ユーザーガイド](docs/USER_GUIDE.md)、設定値の意味は
[パラメーターリファレンス](docs/PARAMETERS.md) を参照してください。

- **♪ 全体生成** — 設定パネル(ダイアレクト・キー・テンポ・構成・シード)の内容から複数候補を内部生成し、品質と多様性を評価した1案を自動表示。同じシードで再実行した場合は新しいシードへ進む
- **他の候補** — 表示中の曲と同じシードから最大4つの別案を作り、差分タグを見ながら同じ再生位置で比較
- **部分再生成** — タイムラインでセクションを選び、全体/メロディ/コード/ベース/伴奏だけを変奏。伴奏の再生成ではベースを保持
...
- **編曲・作曲調整** — 自動編曲、伴奏の厚さ、展開、従来の楽器別パターンと作曲パラメータを編集後に「適用」、または表示中の値で全体生成。ミキサーは即時反映
...
| M3 | 全 14 ダイアレクト・合作モード・譜面表示 (v1.6 で 18、v1.7 で 20 ダイアレクトへ拡張) | ✅ 完了 |
| M4 | WAV 書き出し・仮歌詞・Suno 用テキスト出力 | ✅ 完了 |
| M5 | PWA公開・基本ダイアレクトエディタ・品質保証 | ✅ v1.0 |
| M6 | 候補評価基盤・コード／ベース／伴奏生成・候補比較・3段階の生成根拠 | ✅ v1.2 |
| M7 | Dialect v2（文法の分割・継承・組み合わせ） | 計画中 |
| M8 | 解説動画出力・スタンドアロン化 (Electron / Tauri) | Later |
