...
function noteXml(note: NoteEvent, voice: number): string {
  return `<note>${pitchXml(note.pitch)}<duration>${duration(note.duration)}</duration><voice>${voice}</voice><velocity>${note.velocity}</velocity></note>`;
}
...
function eventsForBar(song: Song, part: SongPart, bar: number): NoteEvent[] {
  ...
  return section[part].filter((note) =>
    note.start >= localBarStart && note.start < localBarStart + song.meter.barBeats)
    .map((note) => ({ ...note, start: note.start - localBarStart }));
}
...
function measureXml(song: Song, part: SongPart, bar: number, first: boolean): string {
  ...
  const notes = eventsForBar(song, part, bar);
  if (notes.length === 0) {
...
export function buildMusicXml(song: Song): string {
  ...
  const parts = PARTS.map(([part], index) => {
    const measures = Array.from({ length: song.totalBars }, (_, bar) => measureXml(song, part, bar, bar === 0)).join("");
